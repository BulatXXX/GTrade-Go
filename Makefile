...
test-all:
	...
	docker compose -f deploy/docker-compose.yml --env-file deploy/.env up -d postgres-catalog postgres-notification
	cd shared/catalogimport && GOCACHE=/tmp/gocache-catalogimport go test ./...
	cd shared/httpmiddleware && GOCACHE=/tmp/gocache-shared go test ./...
	...
	cd services/user-asset-service && GOCACHE=/tmp/gocache-user-asset go test ./...
	...
	cd tools/catalog-importer && GOCACHE=/tmp/gocache-importer go test ./...
