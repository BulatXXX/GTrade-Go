...
func (s *WarframeSource) TotalHint(ctx context.Context) (int, error) {
	items, err := s.fetchItems(ctx)
	if err != nil {
		return 0, err
	}
	return applyLimit(len(items), s.limit), nil
}

func (s *WarframeSource) fetchItems(ctx context.Context) ([]warframeItem, error) {
